...
import { getsubsubCategoriesList } from "@/service/subsubcategories";
...
export default function Home({ params: { id } }: Props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const productData = await getsubsubCategoriesList(parseInt(id));
        console.log("subsubcategoryproductData:", productData);
      ...
      breadcrumbs.push(
        <>
          ...
          <Link key="category" href={`/categories/${category_id}`}>
            <span className="text-yellow-500 hover:text-yellow-700 font-semibold">
              {category_name}
            </span>
          </Link>
        </>
      ...
      if (sub_category_name) {
        breadcrumbs.push(
          <>
            ...
            <Link key="subcategory" href={`/subcategory/${sub_category_id}`}>
              <span className="text-yellow-500 hover:text-yellow-700 font-semibold">
                {sub_category_name}
              </span>
            </Link>
      ...
      if (sub_sub_category_name) {
        breadcrumbs.push(
          <>
            ...
            <Link key="subsubcategory" href={`/subsubcategory`}>
              <span className="text-yellow-500 hover:text-yellow-700 font-bold underline">
                {sub_sub_category_name}
              </span>
            </Link>
          </>
