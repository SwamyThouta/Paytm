...
export default function Home({ params: { id } }: Props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const productData = await getCategoriesList(parseInt(id));
        console.log("categoryproductData:", productData);
  ...
  const renderBreadcrumbs = () => {
    ...
    const category_name = productsList[0]?.category_name;
    return (
      <div className="flex items-center space-x-2 text-gray-600 mb-4">
        ...
        {category_name && (
          <>
            ...
            <Link key="category" href={`/categories/${id}`}>
      ...
      <div className="flex justify-center items-center mt-5 px-6 mb-10">
        {loading ? (
          ...
          <div className="w-full max-w-screen-xl mx-auto">
            <nav className="text-sm mb-4">{renderBreadcrumbs()}</nav>
